...
public class SegmentList {
	...
	public void add(int l_int, int r_int, boolean create_seg) {
		...
		if(pred.is_seg != create_seg)  // insert left. 
			insert(window.pred, new Node(geometryRandom()+1, l, create_seg));
		if(window.pred[0].is_seg != is_seg)  // insert right.
			insert(window.pred, new Node(geometryRandom()+1, r, is_seg));
	}

	private int geometryRandom() {
		int level = 0;
		while(true) {
			if(random.nextInt(2) == 1) {
				if(level == maxLevel-1) return level;
				level = level+1;
			}else{
				return level;
			}
