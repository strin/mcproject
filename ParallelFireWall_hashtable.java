...
public class ParallelFireWall implements Runnable {
	...
	ConcurrentHashMap<Integer, Boolean> tablePng;
	ConcurrentHashMap<Integer, SegmentList> tableR;
	ConcurrentHashMap<Integer, Histogram> histograms;
	ConcurrentHashMap<Integer, ReentrantLock> fineLocks;
	...
	public ParallelFireWall(PaddedPrimitiveNonVolatile<Boolean> done,
		...
		this.tablePng = new ConcurrentHashMap<Integer, Boolean>(1<<numAddressLog);
		this.tableR = new ConcurrentHashMap<Integer, SegmentList>(1<<numAddressLog);
		this.histograms = new ConcurrentHashMap<Integer, Histogram>(1<<numAddressLog);
		this.fineLocks = new ConcurrentHashMap<Integer, ReentrantLock>(1<<numAddressLog);
		this.fingerprint = new Fingerprint();
	}

	private void initFineLockIfNecessary(Integer address) {
		if(this.fineLocks.get(address) == null) 
			this.fineLocks.put(address, new ReentrantLock());
	}
...
class ParallelFireWallWorkerSTM implements Runnable {
	final int id;

	PaddedPrimitiveNonVolatile<Boolean> done;
	LamportQueue<Packet>[] queues;
	Lock[] locksOnQueue;
	ParallelFireWall firewall;
	final int numQueues;
	Fingerprint fingerprint;
	...
	public ParallelFireWallWorkerSTM(PaddedPrimitiveNonVolatile<Boolean> done,
						 ParallelFireWall firewall,
						 LamportQueue<Packet>[] queues,
						 Lock[] locksOnQueue,
						 int id) {
